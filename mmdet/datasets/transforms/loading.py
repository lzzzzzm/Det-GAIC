...
class LoadRGBTIRFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            if self.file_client_args is not None:
                ...
                tir_file_client = fileio.FileClient.infer_client(
                    self.file_client_args, tir_filename)
                img_bytes = file_client.get(rgb_filename)
                tir_bytes = tir_file_client.get(tir_filename)
            else:
                img_bytes = fileio.get(
                    rgb_filename, backend_args=self.backend_args)
                tir_bytes = fileio.get(
                    tir_filename, backend_args=self.backend_args)
            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
            tir_img = mmcv.imfrombytes(
                tir_bytes, flag=self.color_type, backend=self.imdecode_backend)
